<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('pkh', function (Blueprint $table) {
            $table->id();
            $table->foreignId('dtks_id')->constrained('dtks')->onDelete('cascade');
            $table->boolean('ibu_hamil')->default(false);
            $table->boolean('anak_usia_dini')->default(false);
            $table->integer('jumlah_sd')->default(0);
            $table->integer('jumlah_smp')->default(0);
            $table->integer('jumlah_sma')->default(0);
            $table->boolean('disabilitas_berat')->default(false);
            $table->boolean('lanjut_usia')->default(false);
            $table->string('status_penerima')->default('belum aktif');
            $table->string('status')->default('ditinjau');
            $table->text('catatan_peninjau')->nullable();
            $table->text('catatan_surveyor')->nullable();
            $table->text('alasan_tinjauan_kembali')->nullable();
            $table->string('status_kpm')->default('Aktif');
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('pkh');
    }
};
